...
var veiLayer = new L.layerGroup();

// Create map object and set default layers
var myMap = L.map("map", {
  center: [13.0827, 80.2707],
  zoom: 2,
  layers: [dark,veiLayer]
});


d3.json("GeoJSON/volcano.json", parseJSON);
d3.json("GeoJSON/PB2002_boundaries.json", parseBoundariesJSON);
d3.json("GeoJSON/explosivity.geojson", parseExplosivitygeoJSON);
...
function parseExplosivitygeoJSON(response) {
    console.log("volcanic explosivity index response");
    console.log(response);
    console.log( response.features.length);
  for (var i = 0; i < response.features.length; i++) {
    var explosivity = response.features[i];
    var veiCircle = L.circle([explosivity.geometry.coordinates[1], explosivity.geometry.coordinates[0]], {
      opacity: 1,
      fillOpacity: 1,
      fillColor: getColor(explosivity.properties.vei),
      radius: 100000,
      stroke: true,
      weight: 0.2
    }).bindPopup("<h2>" + explosivity.properties.name + "</h2> <hr> <h3> VEI  :" + explosivity.properties.vei + "</h3>").addTo(veiLayer);

    //veiMarkers.push(veiCircle);
   
  }
}

function getRadius(vei) {
  return vei * 100000
}
